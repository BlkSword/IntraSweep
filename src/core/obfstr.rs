...
pub mod sensitive {
    ...
    pub fn vuln_label() -> String {
        xor_decode(&[0xBC, 0xE6, 0xD5, 0xBC, 0xEE, 0xC4, 0xBC, 0xD3, 0xF1, 0xBC, 0xD5, 0xD5], KEY)
    }

    pub fn privesc_label() -> String {
        xor_decode(&[0xBC, 0xD5, 0xCA, 0xBC, 0xC7, 0xD9, 0xBC, 0xF9, 0xDA, 0xBC, 0xEF, 0xD1], KEY)
    }
...
mod tests {
    ...
    #[test]
    fn test_vuln_label() {
        assert_eq!(sensitive::vuln_label(), "漏洞扫描");
    }

    #[test]
    fn test_privesc_label() {
        assert_eq!(sensitive::privesc_label(), "提权检测");
    }
